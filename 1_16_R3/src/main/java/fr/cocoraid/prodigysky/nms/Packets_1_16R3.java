...
public class Packets_1_16R3 implements Packets {
   ...
   public void setSmog(Player player) {
      ...
      if (!dimension.getKey().equalsIgnoreCase("the_nether")) {
         ...
         PacketPlayOutLogin login = new PacketPlayOutLogin(ep.getId(), ep.playerInteractManager.getGameMode(), ep.playerInteractManager.c(), BiomeManager.a(ws.getSeed()), ws.worldData.isHardcore(), ep.getMinecraftServer().F(), ep.getMinecraftServer().customRegistry, ep.getWorldServer().getDimensionManager(), ws.getDimensionKey(), ep.getMinecraftServer().getMaxPlayers(), ws.spigotConfig.viewDistance, flag1, !flag, ws.isDebugWorld(), ws.isFlatWorld());
   ...
   private void refreshChunksAround(Player player) {
      ...
      (new BukkitRunnable() {
         public void run() {
            Packets_1_16R3.refreshChunksMethod.invoke(worldServer.getChunkProvider().playerChunkMap, ep, true);
         }
      }).runTaskLater(ProdigySky.getInstance(), 3L);
   }

   static {
      refreshChunksMethod = Reflection.getMethod(PlayerChunkMap.class, "a", EntityPlayer.class, Boolean.TYPE);
   }
}
